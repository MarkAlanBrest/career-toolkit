...
(function () {
  ...
  if (window.__CE_CONTENT_STUDIO__) return;
  window.__CE_CONTENT_STUDIO__ = true;

  // ── DISABLE / UNINSTALL ─────────────────────────────────────────────────────
  // Tampermonkey gives scripts no API to remove themselves — GM_registerMenuCommand
  // (Tampermonkey icon → this script's commands) plus this early-exit flag is the
  // closest thing to a working "disable" switch. A full removal still requires the
  // Tampermonkey dashboard, which Settings links to directly.
  if (typeof GM_registerMenuCommand === 'function') {
    if (GM_getValue('ce_disabled', false)) {
      GM_registerMenuCommand('▶ Enable Canvas Content Studio', () => {
        GM_setValue('ce_disabled', false);
        window.location.reload();
      });
      return;
    }
    GM_registerMenuCommand('⏸ Disable Canvas Content Studio', () => {
      GM_setValue('ce_disabled', true);
      window.location.reload();
    });
  } else if (GM_getValue('ce_disabled', false)) {
    return;
  }
  ...
  // ── BACKEND (career-toolkit-ruby.vercel.app) ───────────────────────────────────
  const API_BASE = 'https://career-toolkit-ruby.vercel.app';

  // accountId = canvasUserId@canvasDomain when available (matches the Canvas Enhancer
  // extension's scheme) so AI credits are shared across devices/browsers for the same
  // teacher, whether they're using the extension or this userscript. Falls back to a
  // random per-install id when Canvas's ENV isn't available (e.g. non-course pages).
  function getAccountId() {
    const canvasUserId = window.ENV?.current_user_id;
    if (canvasUserId) {
      const canvasAccountId = `${canvasUserId}@${window.location.hostname}`;
      GM_setValue('ce_canvas_account_id', canvasAccountId);
      return canvasAccountId;
    }
    const stored = GM_getValue('ce_canvas_account_id', '');
    if (stored) return stored;
    let id = GM_getValue('ce_install_id', '');
    if (!id) { id = crypto.randomUUID(); GM_setValue('ce_install_id', id); }
    return id;
  }
  function getAccountToken() {
    let token = GM_getValue('ce_account_token', '');
    if (!token) { token = crypto.randomUUID(); GM_setValue('ce_account_token', token); }
    return token;
  }
  ...
  function gmFetch(opts) {
    return new Promise((resolve, reject) => {
      GM_xmlhttpRequest({
        ...
        onerror: () => reject(new Error('Network error contacting Canvas Content Studio server.')),
        ontimeout: () => reject(new Error('Request timed out — please try again.')),
      });
    });
  }

  // ── AI GENERATE (direct call — mirrors the extension's background.js handleGenerate) ──
  async function ceGenerate(payload) {
    const { messages, max_tokens, model, usageType } = payload;
    const body = JSON.stringify({
      messages, max_tokens, model,
      usageType: usageType || (model?.includes('haiku') ? 'teaching' : 'creation'),
      accountId: getAccountId(), accountToken: getAccountToken(),
    });

    const res = await gmFetch({ method: 'POST', url: `${API_BASE}/api/generate`, headers: { 'Content-Type': 'application/json' }, body });

    if (res.status < 200 || res.status >= 300) {
      let errData = {};
      try { errData = JSON.parse(res.responseText); } catch {}
      throw new Error(errData?.error || `HTTP ${res.status}`);
    }

    const rawBody = res.responseText || '';
    let fullText = '';
    let gotChunks = false;
  ...
  async function ceCreditStatus() {
    const accountId = getAccountId(), accountToken = getAccountToken();
    const res = await gmFetch({ url: `${API_BASE}/api/credits/status?accountId=${encodeURIComponent(accountId)}&accountToken=${encodeURIComponent(accountToken)}` });
    let data = {};
  ...
  const PAGE_TYPES = [
    ...
    { value:'quiz',         label:'Quiz Instructions', icon:'✅' },
  ];

  const CONTENT_SUBTYPES = {
    page: [
      { value:'',         label:'General Page' },
      { value:'info',     label:'Info / Overview Page' },
      { value:'lesson',   label:'Content / Lesson Page' },
      { value:'review',   label:'Review / Study Guide' },
      { value:'video',    label:'Video Page' },
      { value:'welcome',  label:'Welcome / Course Intro' },
      { value:'resource', label:'Resource / Links Page' },
    ],
    assignment: [
      { value:'',         label:'General Assignment' },
      { value:'essay',    label:'Essay / Written Response' },
      { value:'research', label:'Research Project' },
      { value:'present',  label:'Presentation' },
      { value:'lab',      label:'Lab / Hands-On Activity' },
      { value:'creative', label:'Creative Work' },
      { value:'case',     label:'Case Study' },
      { value:'group',    label:'Group Project' },
    ],
    announcement: [
      { value:'',          label:'General Announcement' },
      { value:'reminder',  label:'Missing Work / Deadline Reminder' },
      { value:'welcome',   label:'Welcome / Course Intro' },
      { value:'event',     label:'Upcoming Event' },
      { value:'update',    label:'Course Update' },
    ],
  };

  const SUBTYPE_DESCS = {
    page: {
      info:     'an informational overview page — clear, scannable, with key facts and context',
      lesson:   'a content/lesson page — structured learning material with explanations and examples',
      review:   'a review and study guide — key terms, summaries, and practice questions',
      video:    'a video page — embedded video as the centerpiece, with supporting context and notes',
      welcome:  'a welcome/intro page — warm, inviting course introduction with instructor info and what to expect',
      resource: 'a resource and links page — organized collection of materials, references, and external links',
    },
    assignment: {
      essay:    'an essay or written response assignment — clear prompt, length and format requirements, grading criteria',
      research: 'a research project — multi-step with source requirements, outline, and bibliography expectations',
      present:  'a presentation assignment — slides or oral delivery, structure, rubric, and timing guidance',
      lab:      'a lab or hands-on activity — procedure, materials, data collection, and conclusion requirements',
      creative: 'a creative work assignment — portfolio piece, story, design brief, or artistic project',
      case:     'a case study — real-world scenario with analysis questions and a written recommendation',
      group:    'a group project — team roles, milestones, shared deliverables, and individual accountability',
    },
    announcement: {
      reminder: 'a missing work or deadline reminder — polite but direct, with specific next steps for students',
      welcome:  'a welcome announcement — warm, encouraging, sets expectations and links to key resources',
      event:    'an upcoming event or activity announcement — what, when, where, and why it matters',
      update:   'a course update — change in schedule, policy, or assignment, with clear explanation',
    },
  };
  ...
  function showContentBuilder() {
    ...
    function render() {
      ...
      if      (st.view==='build')   body.appendChild(cbBuild());
    ...
    function cbHeader() {
      ...
      t.textContent = st.view==='result' ? 'Review Content' : 'AI Assist';
    ...
    function cbBuild() {
      ...
      typeSel.onchange=()=>{ st.contentType=typeSel.value; st.subType=''; render(); };
      typeCard.appendChild(typeSel);

      const subtypes = CONTENT_SUBTYPES[st.contentType];
      if (subtypes) {
        const subLbl=document.createElement('div'); subLbl.style.cssText='font-size:11px;color:#9ca3af;margin:8px 0 4px;';
        subLbl.textContent='Type';
        const subSel=document.createElement('select');
        subSel.style.cssText='width:100%;padding:10px 12px;border-radius:8px;border:1px solid #d1d5db;background:#fff;font-size:13px;font-family:inherit;cursor:pointer;color:#374151;';
        subtypes.forEach(s=>{ const o=document.createElement('option'); o.value=s.value; o.textContent=s.label; if(s.value===st.subType) o.selected=true; subSel.appendChild(o); });
        subSel.onchange=()=>{ st.subType=subSel.value; };
        typeCard.appendChild(subLbl);
        typeCard.appendChild(subSel);
      }
  ...
  function cbGenerate(st, genBtn, renderFn) {
    ...
    const effectiveDesc = (st.subType && SUBTYPE_DESCS[st.contentType]?.[st.subType]) || typeDesc[st.contentType] || typeDesc.page;
    let prompt=`You are an expert Canvas LMS content designer. Generate professional HTML for ${effectiveDesc}.\n\n`;
...
const s=raw.indexOf('{'),e=raw.lastIndexOf('}');
  ...
  // ── SETTINGS / HELP / AI CREDITS (self-contained — no hub.js dependency) ───────
  function ceModal(titleText, bodyEl, width) {
    const existing = document.getElementById('ce-settings-overlay');
    if (existing) existing.remove();
    const overlay = document.createElement('div');
    overlay.id = 'ce-settings-overlay';
    ...
    overlay.addEventListener('click', e => { if (e.target === overlay) overlay.remove(); });
    const panel = document.createElement('div');
    panel.style.cssText = `background:#fff;border-radius:10px;width:${width || 420}px;max-width:calc(100vw - 32px);max-height:85vh;overflow-y:auto;box-shadow:0 12px 40px rgba(0,0,0,.3);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;`;
    const hdr = document.createElement('div');
    hdr.style.cssText = 'display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #eee;';
    const h = document.createElement('strong'); h.textContent = titleText; h.style.cssText = 'font-size:15px;color:#172A36;';
    const x = document.createElement('button'); x.textContent = '✕'; x.type = 'button';
    x.style.cssText = 'background:none;border:none;color:#94a3b8;font-size:18px;cursor:pointer;padding:2px;line-height:1;';
    x.onclick = () => overlay.remove();
    hdr.append(h, x);
    const body = document.createElement('div'); body.style.cssText = 'padding:18px 20px;';
  ...
  function openSettingsModal() {
    const w = document.createElement('div');
    const lblStyle = 'font-size:12px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:.04em;margin-bottom:6px;';

    const modelLabel = document.createElement('div'); modelLabel.textContent = 'AI Model'; modelLabel.style.cssText = lblStyle;
    const modelSel = document.createElement('select');
    modelSel.style.cssText = 'width:100%;box-sizing:border-box;padding:9px 11px;border:1px solid #ccc;border-radius:6px;font-size:13px;font-family:inherit;background:#fff;margin-bottom:18px;cursor:pointer;';
    const curModel = getContentModel();
    CONTENT_MODELS.forEach(m => {
      const o = document.createElement('option'); o.value = m.id; o.textContent = `${m.label} — ${m.detail}`;
      if (m.id === curModel) o.selected = true;
      modelSel.appendChild(o);
    });

    const status = document.createElement('div'); status.style.cssText = 'font-size:12px;min-height:16px;margin-bottom:10px;color:#127A1B;font-weight:600;';

    const saveBtn = document.createElement('button'); saveBtn.type = 'button'; saveBtn.textContent = 'Save';
    saveBtn.style.cssText = 'width:100%;padding:11px;background:#0770B8;color:#fff;border:none;border-radius:6px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;';
    saveBtn.onclick = () => {
      GM_setValue('ce_content_model', modelSel.value);
      status.textContent = '✓ Saved';
      setTimeout(() => { status.textContent = ''; }, 2500);
    };

    // ── DANGER ZONE ──────────────────────────────────────────────────────────
    const dzDivider = document.createElement('div'); dzDivider.style.cssText = 'height:1px;background:#f1f5f9;margin:18px 0 14px;';
    const dzLabel = document.createElement('div'); dzLabel.textContent = 'Disable / Uninstall'; dzLabel.style.cssText = lblStyle;
    const dzCopy = document.createElement('div');
    dzCopy.style.cssText = 'font-size:12px;color:#6b7280;line-height:1.6;margin-bottom:10px;';
    dzCopy.textContent = 'Turning this off stops the toolbar from loading on any Canvas page until you turn it back on from the Tampermonkey menu. To remove the script completely, use the Tampermonkey dashboard.';
    const disableBtn = document.createElement('button'); disableBtn.type = 'button'; disableBtn.textContent = 'Disable Content Studio';
    disableBtn.style.cssText = 'width:100%;padding:10px;background:#fff;color:#B91C1C;border:1px solid #FCA5A5;border-radius:6px;font-size:13px;font-weight:700;cursor:pointer;font-family:inherit;margin-bottom:8px;';
    disableBtn.onclick = () => {
      if (!confirm('Disable Canvas Content Studio? The toolbar will stop appearing on Canvas pages until you re-enable it from the Tampermonkey extension menu.')) return;
      GM_setValue('ce_disabled', true);
      window.location.reload();
    };
    const uninstallLink = document.createElement('a');
    uninstallLink.href = 'https://www.tampermonkey.net/faq.php#Q209';
    uninstallLink.target = '_blank'; uninstallLink.rel = 'noopener noreferrer';
    uninstallLink.textContent = 'How to fully remove it from Tampermonkey →';
    uninstallLink.style.cssText = 'display:block;text-align:center;font-size:12px;color:#0770B8;text-decoration:none;';

    w.append(modelLabel, modelSel, status, saveBtn, dzDivider, dzLabel, dzCopy, disableBtn, uninstallLink);
    ceModal('Content Studio Settings', w);
  }

  function openHelpModal(context) {
    const copy = context === 'quiz'
      ? 'Quiz Maker: enter a topic, choose question types, difficulty, and how many variants per question, then generate. Review the generated questions, select the ones you want, and export them straight into a Canvas quiz (Classic or New Quizzes).'
      : 'Content Studio: use Insert, Layouts, and Icons to drop styled components into the Canvas rich content editor — free, no account needed. AI Assist has Claude draft a full page, assignment, or discussion prompt from a topic, an uploaded file, or the current editor content, and costs AI credits — buy more any time from AI Credits (✨ AI Assist ▾).';
    const w = document.createElement('div');
    w.style.cssText = 'font-size:13px;line-height:1.6;color:#374151;';
    w.textContent = copy;
    ceModal('How Content Studio Works', w);
  }

  // Buy Credits — mirrors the extension's hub.js openAICredits() purchase flow:
  // an embedded Stripe checkout iframe on career-toolkit-ruby.vercel.app/buy-credits,
  // identified by accountId/accountToken, that posts CE_PAYMENT_SUCCESS back on completion.
  let creditsPaymentHandler = null;
  ...
  async function openAICreditsModal() {
    const w = document.createElement('div');
    w.style.cssText = 'display:flex;flex-direction:column;gap:14px;font-size:13px;color:#374151;';

    const balanceCard = document.createElement('div');
    balanceCard.style.cssText = 'padding:14px 16px;border:1px solid #e5e7eb;border-radius:8px;background:#F8FAFC;display:flex;align-items:center;justify-content:space-between;gap:12px;';
    const balanceLeft = document.createElement('div');
    const balanceLabel = document.createElement('div'); balanceLabel.textContent = 'AI Credit Balance';
    balanceLabel.style.cssText = 'font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;';
    const balanceText = document.createElement('div'); balanceText.textContent = '…';
    balanceText.style.cssText = 'font-size:32px;font-weight:900;color:#111827;line-height:1;';
    const statusText = document.createElement('div'); statusText.textContent = 'Loading balance…';
    statusText.style.cssText = 'font-size:12px;color:#6b7280;margin-top:4px;';
    balanceLeft.append(balanceLabel, balanceText, statusText);
    const costsList = document.createElement('div');
    costsList.style.cssText = 'font-size:11px;color:#6b7280;line-height:1.8;text-align:right;';
    costsList.innerHTML = 'Page: 3–10 cr<br>Quiz: 3–10 cr';
    balanceCard.append(balanceLeft, costsList);

    let selectedPack = 'teacher';
    const packRow = document.createElement('div');
    packRow.style.cssText = 'display:grid;grid-template-columns:repeat(2,1fr);gap:8px;';
    function renderPackButtons() {
      packRow.innerHTML = '';
      CREDIT_PACKS.forEach(p => {
        const active = selectedPack === p.key;
        const b = document.createElement('button'); b.type = 'button';
        b.style.cssText = `padding:10px 6px;border:2px solid ${active ? '#0770B8' : '#e5e7eb'};border-radius:7px;background:${active ? '#EDF5FF' : '#fff'};cursor:pointer;text-align:center;font-family:inherit;transition:all .12s;`;
        b.innerHTML = `<div style="font-size:15px;font-weight:800;color:${active ? '#0770B8' : '#111827'};">${p.price}</div><div style="font-size:11px;color:#6b7280;margin-top:2px;">${p.credits}</div>`;
        b.onclick = () => { selectedPack = p.key; renderPackButtons(); };
    ...
    saveCardRow.style.cssText = 'display:flex;align-items:center;gap:10px;cursor:pointer;font-size:12px;color:#374151;';
    const saveCardCheck = document.createElement('input'); saveCardCheck.type = 'checkbox';
    saveCardCheck.onchange = () => { saveCard = saveCardCheck.checked; };
    saveCardRow.append(saveCardCheck, document.createTextNode('Save card for automatic top-ups'));

    const frameWrap = document.createElement('div');
    frameWrap.style.cssText = 'display:none;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;';

    const buyBtn = document.createElement('button'); buyBtn.type = 'button'; buyBtn.textContent = 'Pay with Card';
    buyBtn.style.cssText = 'width:100%;padding:11px;background:#0770B8;color:#fff;border:none;border-radius:6px;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit;';

    let accountId = '', accountToken = '';

    function showCheckoutFrame() {
      frameWrap.innerHTML = '';
      const src = `${API_BASE}/buy-credits?accountId=${encodeURIComponent(accountId)}&accountToken=${encodeURIComponent(accountToken)}&pack=${selectedPack}${saveCard ? '&saveCard=true' : ''}`;
      const iframe = document.createElement('iframe');
      iframe.src = src; iframe.style.cssText = 'width:100%;height:580px;border:none;display:block;';
      frameWrap.appendChild(iframe);
      balanceCard.style.display = 'none'; packRow.style.display = 'none'; saveCardRow.style.display = 'none'; buyBtn.style.display = 'none';
      frameWrap.style.display = 'block';
    }
    function hideCheckoutFrame() {
      frameWrap.style.display = 'none'; frameWrap.innerHTML = '';
      balanceCard.style.display = ''; packRow.style.display = ''; saveCardRow.style.display = ''; buyBtn.style.display = 'block';
    }
    buyBtn.onclick = () => { if (accountId && accountToken) showCheckoutFrame(); };
    ...
    function onPaymentMessage(e) {
      ...
      if (e.data?.type === 'CE_PAYMENT_SUCCESS') {
        ...
        creditsPaymentHandler = null;
        hideCheckoutFrame();
        const added = Number(e.data?.credits || 0);
        const current = Number(String(balanceText.textContent).replace(/,/g, '')) || 0;
        ...
        statusText.style.color = '#127A1B';
        setTimeout(() => { statusText.style.color = '#6b7280'; statusText.textContent = `${newBalance.toLocaleString()} total credits`; }, 3000);
      }
    ...
    if (creditsPaymentHandler) window.removeEventListener('message', creditsPaymentHandler);
    creditsPaymentHandler = onPaymentMessage;
    window.addEventListener('message', onPaymentMessage);

    w.append(balanceCard, packRow, saveCardRow, buyBtn, frameWrap);
    ceModal('AI Credits', w, 580);

    try {
      const data = await ceCreditStatus();
      accountId = data.accountId || getAccountId();
      accountToken = data.accountToken || getAccountToken();
      const bal = Number(data.balance || 0);
      balanceText.textContent = bal.toLocaleString();
      statusText.textContent = `${bal.toLocaleString()} total credits`;
  ...
  document.addEventListener('ce-open-settings', openSettingsModal);
  document.addEventListener('ce-open-help', e => openHelpModal(e.detail));
  document.addEventListener('ce-open-ai-credits', () => { openAICreditsModal(); });
