...
export function evaluateAccscFlags(records: CareerRecord[]): AccscFlag[] {
  ...
  for (const record of records) {
    ...
    if (isEmployedInField(record)) {
      const hasPhoneOrEmail = record.employerPhone || record.employerEmail;
      for (const { field, label, rule } of EMPLOYMENT_DOC_FIELDS) {
        const value = record[field];
        if (typeof value === 'string' && !value.trim()) {
          if (field === 'employerPhone' || field === 'employerEmail') {
            if (!hasPhoneOrEmail) {
              flags.push({
                severity: 'error',
                recordId: record.id,
                ...
                field: 'employerPhone',
                message: 'Missing employer phone or email for employment verification.',
                rule,
              });
            }
            continue;
