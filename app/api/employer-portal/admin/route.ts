...
export async function POST(request: NextRequest) {
  ...
  if (action === 'startMicrosoftLogin') {
    ...
    if (!settings.microsoftTenantId || !settings.microsoftClientId) {
      return NextResponse.json({
        error: 'Configure the Microsoft tenant and application IDs in Email settings first.',
      }, { status: 400 });
    }
  ...
  if (action === 'pollMicrosoftLogin') {
    ...
    if (!(await isMicrosoftAdminAllowed(email))) {
      return NextResponse.json({
        error: `${email} is not on the Employer Portal administrator list.`,
      }, { status: 403 });
    }
    ...
    if (needsSenderConnection) {
      ...
      await saveEmployerPortalSettings({
        ...settings,
        senderEmail: email,
        senderName: profile.displayName?.trim() || settings.senderName || 'NCST Career Services',
