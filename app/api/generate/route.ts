...
import { refundGeneration, reserveGeneration } from '@/lib/billing';
...
export async function POST(req: NextRequest) {
  ...
  const entitlement = await reserveGeneration(licenseKey);
  ...
  if (!apiKey) {
    await refundGeneration(licenseKey, entitlement.chargedFrom);
  ...
  return new NextResponse(anthropicRes.body, { status: 200, headers: {
    ...
    'X-CE-Plan': entitlement.plan || '', 'X-CE-Remaining': String(entitlement.usage?.remaining ?? ''), 'X-CE-Bonus': String(entitlement.usage?.bonus ?? 0),
  } });
}
