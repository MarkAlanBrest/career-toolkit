...
function renderActivity(block){
 ...
 if(block.videoUrl){const mediaClass=block.layoutStyle==="split"||block.layoutStyle==="spotlight"?"media media-stack":"media";const frame='<iframe src="'+block.videoUrl+'" title="'+block.title+'" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';const caption='<div>'+(block.caption?'<p class="caption">'+block.caption+'</p>':'')+'</div>';html+='<div class="'+mediaClass+'">'+(block.layoutStyle==="media-left"?frame+caption:caption+frame)+'</div>'}
