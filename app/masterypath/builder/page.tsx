...
function renderActivity(block){
 ...
 if(block.videoUrl){html+=renderVideo(block)}
...
function renderVideo(block){const src=safeAttr(block.videoUrl);const frame='<iframe src="'+src+'" title="'+safeAttr(block.title||"Video")+'" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><p class="caption"><a href="'+src+'" target="_blank" rel="noopener">Open video</a></p>';const caption=block.caption?'<p class="caption">'+block.caption+'</p>':'';return '<div class="video-frame">'+frame+caption+'</div>'}
...
function renderSlideContent(block){
 ...
 if(block.videoUrl&&style==="spotlight"){return title+body+'<div class="video-full">'+renderVideo(block)+'</div>'}
 if(block.videoUrl&&style==="split"){return '<div class="slide-split"><div>'+title+body+'</div><div>'+renderVideo(block)+'</div></div>'}
 if(block.videoUrl&&style==="media-left"){return '<div class="slide-split media-first"><div>'+renderVideo(block)+'</div><div>'+title+body+'</div></div>'}
