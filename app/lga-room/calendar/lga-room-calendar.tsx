...
export function LgaRoomCalendar({ embedded = false }: { embedded?: boolean }) {
  ...
  const showAdminControls = !embedded;
  ...
  const calendarBody = (
    <>
      ...
      <section style={{ maxWidth: embedded ? 'none' : 840, margin: '0 auto', padding: embedded ? 0 : '18px 16px 24px' }}>
        <div
          role="note"
          style={{
            display: 'flex',
            ...
            borderRadius: 10,
            color: accentStrong,
            boxShadow: embedded ? 'none' : '0 4px 14px rgba(0,45,116,0.06)',
          }}
        >
          <span
            aria-hidden="true"
        ...
        <div style={{ background: surface, border: `1px solid ${border}`, borderRadius: 14, overflow: 'hidden', boxShadow: embedded ? '0 6px 18px rgba(0,31,82,.055)' : '0 1px 2px rgba(32,36,31,0.04), 0 10px 28px rgba(32,36,31,0.06)' }}>
      ...
      {showAdminControls && showAdminLogin && (
        <AdminLoginModal
  ...
  if (embedded) {
    return <div style={{ color: text }}>{calendarBody}</div>;
  }
