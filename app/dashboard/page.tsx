...
function Content() {
  ...
  const folder = d.CourseName.replace(/\s+/g, "");
  ...
  return (
    <main className="min-h-screen bg-slate-300 flex items-center justify-center">
      <div className="bg-white p-8 rounded-xl shadow-xl w-[520px] space-y-3">
        ...
        <button
          onClick={() => router.push(`/courses/${folder}?code=${encodeURIComponent(code)}`)}
          className="mt-4 w-full bg-blue-900 text-white p-3 rounded-lg hover:bg-blue-800"
        >
          Start Course
        </button>
      </div>
