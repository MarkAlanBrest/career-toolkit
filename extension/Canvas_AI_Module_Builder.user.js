...
(function () {
    ...
    function withTimeout(promise, ms, label){
        return new Promise(function(resolve, reject){
            var timer = setTimeout(function(){
                reject(new Error((label||"Operation")+" timed out after "+Math.round(ms/1000)+"s — the file may be too large, corrupted, or a required library failed to load."));
            }, ms);
    ...
    async function parseFile(file){
        ...
        if(n.endsWith(".pdf"))return withTimeout(parsePDF(file), 180000, "Parsing "+file.name);
        if(n.endsWith(".docx"))return withTimeout(parseDOCX(file), 180000, "Parsing "+file.name);
        if(n.endsWith(".pptx"))return withTimeout(parsePPTX(file), 180000, "Parsing "+file.name);
    ...
    function callClaude(prompt,model,maxTok){
        return new Promise(function(resolve,reject){
            ...
            GM_xmlhttpRequest({
                ...
                data:JSON.stringify({model:model||AI_MODEL_CONTENT,max_tokens:maxTok||TOKENS_DEFAULT,messages:[{role:"user",content:prompt}]}),
                timeout:600000,
                onload:function(resp){
                ...
                ontimeout:function(){reject(new Error("Timed out after 10 minutes — try Standard mode or reduce content"));}
            });
        });
    }
    ...
    function unsplashSearch(keyword){
        return new Promise(function(resolve,reject){
            ...
            GM_xmlhttpRequest({
                method:"GET",
                url:"https://api.unsplash.com/search/photos?per_page=1&query="+encodeURIComponent(keyword),
                headers:{"Authorization":"Client-ID "+state.unsplashKey,"Accept-Version":"v1"},
                timeout:15000,
                onload:function(resp){
                    var d;try{d=JSON.parse(resp.responseText);}catch(e){reject(new Error("Invalid Unsplash response"));return;}
                    if(resp.status!==200){reject(new Error(d&&d.errors?d.errors.join(", "):"HTTP "+resp.status));return;}
                    var photo=d.results&&d.results[0];
                    if(!photo){reject(new Error("No results for \""+keyword+"\""));return;}
                    resolve({
                ...
                onerror:function(){reject(new Error("Network error contacting Unsplash"));},
                ontimeout:function(){reject(new Error("Unsplash request timed out"));}
            });
        });
    }
